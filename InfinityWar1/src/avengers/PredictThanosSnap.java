...
public class PredictThanosSnap {
    ...
    public static void main(String[] args) {
        ...
        boolean[] visited = new boolean[StdIn.readInt()];

        int[][] neighborhood = new int[visited.length][visited.length];
        ...
        visited[beginner] = true;

        dfs(beginner, visited, neighborhood);

        StdOut.setFile(predictThanosSnapOutputFile);

        boolean connected = true;
        for (int i = 0; i < visited.length; i++) {
            if (!del.contains(i) && !visited[i]) {
                connected = false;
                break;
            }
        }
